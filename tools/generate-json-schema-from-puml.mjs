#!/usr/bin/env node
// generate-json-schema-from-puml.mjs  (v1.3)
// Class-schemas-only generator: emits JSON Schemas for classes in PUML with SCHEMAHINTS.
// It DOES NOT write enum JSON files. Enum JSON + enum *.schema.json wrappers come from
...
//   xor: [[a, b], [c, d, e]]                    // mutually-exclusive field groups
//   atleastone: (a|b|c) or [a, b, c]           // at least one of these fields present
//   oneOf: ["/seg/Sub/Name", ...]              // composition with other schemas
//   anyOf: ["/seg/Sub/Name", ...]
//   allOf: ["/seg/Sub/Name", ...]
//
...
//   enumDefine:    { enumId|path: /seg/subs/Name, ... }          // legacy: sets $ref to <baseId>seg/subs/Name.json
//   enumValuesFrom: /seg/subs/EnumName                          // NEW: marks this field as string/number backed by external enum JSON
//                                                                //       -> emits type + x-enumValuesFrom + default (if provided)
...
// --- enum PUML handling ----------------------------------------------------
// Enum PUML files (by convention: *Enum.puml OR containing a top-level `enum <Name> { ... }`)
// are handled by generate_json-enums-from-puml.mjs. We intentionally skip generating class
// schemas for enums here to avoid producing unusable wrapper object schemas (e.g. {encoding: ...}).
function isEnumPuml(absPath, text) {
  const base = path.basename(absPath);
  if (/Enum\.puml$/i.test(base)) return true;
  // crude but effective: a non-comment line starting with `enum <Ident>`
  const lines = String(text).split(/\r?\n/);
  for (const ln of lines) {
    const s = ln.trim();
    if (!s || s.startsWith("'") || s.startsWith("//")) continue;
    if (/^enum\s+[A-Za-z_][A-Za-z0-9_]*\b/.test(s)) return true;
    // stop scanning once we see a class/diagram body
    if (/^(class|abstract\s+class|interface)\b/.test(s)) return false;
  }
  return false;
}

// --- regex normalization (avoid double quotes/backslashes in JSON Schema "pattern") ---
...
if (!baseId) {
  console.error(
    'ERROR: --baseId is required (e.g., --baseId "https://schemas.example.org/hatpro/")'
  );
  process.exit(1);
}
...
function parseNotes(text) {
  const blocks = [];
  const re =
    /note\s+(?:right|left|over)\s+of\s+([A-Za-z_][A-Za-z0-9_]*)[\s\S]*?end note/gi;
  let m;
...
function parseSchemaHints(block) {
  ...
  for (let idx = 0; idx < lines.length; idx++) {
    ...
    const fieldStart = line.match(
      /^\s*field\s+([A-Za-z_][A-Za-z0-9_]*)\s*:\s*$/
    );
    if (fieldStart) {
      currentField = fieldStart[1];
      hints.fields[currentField] = {};
      continue;
    }
    ...
    if (kv) {
      ...
      if (currentField && key === "items" && val === "") {
        const obj = {};
        idx = readNestedObject(lines, idx + 1, obj);
        hints.fields[currentField].items = obj;
        continue;
      }

      // arrays inline (e.g., required:[a,b], allOf:[/seg/A])
      if (/^\[.*\]$/.test(val)) {
        const arr = val
          .slice(1, -1)
          .split(",")
...
function readNestedObject(lines, startIdx, outObj) {
  ...
  for (let j = startIdx; j < lines.length; j++) {
    ...
    if (/^\s*end note\b/i.test(raw)) {
      idx = j - 1;
      break;
    }
    if (/^\s*field\s+[A-Za-z_]/.test(raw)) {
      idx = j - 1;
      break;
    }

    // determine indentation; stop when we step back out of the nested block
...
function buildSchema(hostClass, hints, fileRel, baseId) {
  ...
  const schema = {
    ...
    additionalProperties:
      typeof hints.root.additionalProperties === "boolean"
        ? hints.root.additionalProperties
        : false,
    properties: {},
  };
  ...
  // Root-level logical constraints from SCHEMAHINTS.
  const pushRefList = (key) => {
    const list = hints.root[key];
    if (!Array.isArray(list) || !list.length) return;
    schema[key] = schema[key] || [];
    for (const raw of list) {
      if (!raw) continue;
      const refPath = String(raw).replace(/^\//, "");
      schema[key].push({ $ref: baseId + refPath + ".schema.json" });
    }
  };
  ...
  // xor groups -> schema.oneOf with required sets
  if (hints.root.xor !== undefined) {
    const raw = hints.root.xor;
    const groups = [];
    ...
    if (groups.length) {
      ...
      for (const names of groups) {
        schema.oneOf.push({ required: names });
      }
    }
  }

  // atleastone -> schema.anyOf with single-field required clauses
  if (hints.root.atleastone !== undefined) {
    let names = [];
    const val = hints.root.atleastone;
    if (Array.isArray(val)) {
      names = val.map(String).map((s) => s.trim()).filter(Boolean);
    ...
    if (names.length) {
      ...
      for (const n of names) {
        schema.anyOf.push({ required: [n] });
      }
    }
  }

  // Helper to derive enum JSON file path from enumValuesFrom id
  function enumValuesFromToJsonPath(enumIdLike) {
    if (!enumIdLike) return undefined;
    const clean = String(enumIdLike).trim().replace(/^\//, "");
    if (!clean) return undefined;
    const parts2 = clean.split("/").filter(Boolean);
    if (!parts2.length) return undefined;
    const seg2 = parts2[0];
    const name2 = parts2[parts2.length - 1];
    const subs2 = parts2.slice(1, -1);
  ...
  for (const [fname, spec] of Object.entries(hints.fields || {})) {
    ...
    if (spec.pattern) {
      f.pattern = normalizePattern(spec.pattern);
    } else if (spec.valuePattern) {
      f.pattern = normalizePattern(spec.valuePattern);
    }

    if (spec["$ref"]) {
      const refPath = String(spec["$ref"]).replace(/^\//, "");
      f["$ref"] = baseId + refPath + ".schema.json";
    } else if (spec.enumValuesFrom) {
      const t = spec.type || "string";
      f.type = t;
      const enumJsonPath = enumValuesFromToJsonPath(spec.enumValuesFrom);
      if (enumJsonPath) f["x-enumValuesFrom"] = enumJsonPath;
      // default (if any) was already copied above
    } else if (spec.enumDefine && (spec.enumDefine.enumId || spec.enumDefine.path)) {
      const enumId = String(
        spec.enumDefine.enumId || spec.enumDefine.path
      ).replace(/^\//, "");
...
function outPathFor(hostClass, seg, subs) {
  return path.join(
    ...
    "schemas",
    ...subs,
    `${hostClass}.schema.json`
  );
}

function processFile(absPath, rootDir, baseId) {
  const text = fs.readFileSync(absPath, "utf-8");
  if (isEnumPuml(absPath, text)) return; // enums handled by generate_json-enums-from-puml.mjs
...
function main() {
  ...
  if (singleFile) {
    // Single-file mode wins
    files = [path.resolve(process.cwd(), singleFile)];
  } else if (segment) {
    // Segment mode: only scan ./packages/<segment>/**
    const segRoot = path.join(root, segment);
    if (!fs.existsSync(segRoot)) {
      console.warn(`Segment directory not found: ${segRoot}`);
      return;
    }
    files = listPumlFiles(segRoot);
  } else {
    // Default: all PUML under ./packages
    files = listPumlFiles(root);
  }

  if (!files.length) {
    console.warn(
      `No .puml files found. Root=${root} segment=${segment || ""} file=${singleFile || ""}`
    );
    return;
  }
